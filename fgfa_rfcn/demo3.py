...
from lib.dataset.data_loader import DataLoader
...
def main():
    ...
    data_batch = mx.io.DataBatch(data=[data_idx_], label=[], pad=0, index=idx,
                                 provide_data=[[(k, v.shape) for k, v in zip(data_names, data_idx_)]],
                                 provide_label=[None])
    scales = [data_batch.data[i][1].asnumpy()[0, 2] for i in xrange(len(data_batch.data))]
    all_boxes = [[[] for _ in range(len_data)]
